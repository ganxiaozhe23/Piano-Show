    ...
    *,
    ...
    try:
        with ZipFile(temporary, "w", compression=ZIP_DEFLATED, compresslevel=9) as archive:
            ...
            archive.writestr("working/events.json", _json_bytes(events or []))
...
def read_project(path_or_bytes: str | Path | bytes) -> dict[str, object]:
    ...
    with ZipFile(io.BytesIO(raw)) as archive:
        ...
        events = json.loads(archive.read("working/events.json")) if "working/events.json" in names else []
